...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	//create new semaphore object and initialize it by the given info (ownerID, name, value)
		struct Semaphore *semaphore_object;
		//Return:
		...
		int32 index=allocate_semaphore_object(&semaphore_object);
		for(int i=0;i<MAX_SEMAPHORES;i++)
		{
			if(semaphore_object!=NULL&&index==i)
			{
			    semaphores[index].ownerID=ownerEnvID;
			    strcpy( semaphores[index].name,semaphoreName);
			    semaphores[index].value=initialValue;
			    int32 ret= get_semaphore_object_ID(ownerEnvID,semaphoreName);
			    if(ret==E_SEMAPHORE_EXISTS)
			    {
			    	 return ret;
			    }
