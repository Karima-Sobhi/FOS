...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	struct Env* myenv = curenv; //The calling environment
		...
		int shared_obj_id = allocate_share_object(&allocatedObject);
		if(shared_obj_id==E_NO_SHARE)
		{
		//	free_share_object(shared_obj_id);
				return E_NO_SHARE ;
		}
		if((get_share_object_ID(ownerID,shareName))>=0)
		{
			free_share_object(shared_obj_id);
			return E_SHARED_MEM_EXISTS;
		}
//		int res = allocate_chunk(myenv->env_page_directory,(int)(virtual_address),size/PAGE_SIZE,PERM_USER | PERM_WRITEABLE);
//		 if(res==-1)
//			 return E_SHARED_MEM_EXISTS ;
		...
		for (int i = 0; i < (size/PAGE_SIZE); i++)
		{
			...
			if (ret !=0)
				return E_NO_MEM ;
			map_frame(myenv->env_page_directory, ptr_new_frame,(int)(virtual_address + (i * PAGE_SIZE)), PERM_USER | PERM_WRITEABLE);
			add_frame_to_storage(shares[shared_obj_id].framesStorage, ptr_new_frame, i);
		 }
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	struct Env* myenv = curenv; //The calling environment
		...
		for(int i=0;i<size/PAGE_SIZE;i++)
		{
			struct FrameInfo* ptr_new_frame = get_frame_from_storage(shares[obj_id].framesStorage,i);
